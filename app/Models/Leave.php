...
class Leave extends Model
{
    ...
    protected $casts = [
        ...
        'equivalent_days_csc' => 'decimal:4',
        'service_credits_used' => 'decimal:4',
        'lwop_deduction_rate' => 'decimal:4',
        'lwop_days_charged' => 'decimal:4',
    ...
    protected static function booted()
    {
        ...
        static::created(function ($leave) {
            // Auto-compute CSC equivalent days
            $leave->computeCscEquivalentDays();
        });

        static::updating(function ($leave) {
            // Recompute if dates or hours changed
            if ($leave->isDirty(['start_date', 'end_date', 'duration_type', 'total_hours', 'start_time', 'end_time'])) {
                $leave->computeCscEquivalentDays();
            }
        });
    }
    ...
    /**
     * Compute CSC equivalent days
     */
    public function computeCscEquivalentDays(): void
    {
        if ($this->total_hours && $this->user) {
            $workHoursPerDay = $this->user->work_hours_per_day ?? 8.0;
            $this->equivalent_days_csc = round($this->total_hours / $workHoursPerDay, 4);
        } else {
            $this->equivalent_days_csc = $this->days;
        }
    }
    ...
    /**
     * Get total hours for half-day leaves
     */
    public function getTotalHoursAttribute(): float
    {
        if ($this->attributes['total_hours']) {
            return (float) $this->attributes['total_hours'];
        }

        if ($this->duration_type === self::DURATION_HALF_DAY) {
            return $this->half_day_time === self::HALF_DAY_MORNING ? 4.0 : 4.0;
        }
    ...
    {
        return $this->equivalent_days_csc ?? $this->days;
    }
    ...
    public function calculateLwopDeduction(): array
    {
        ...
        $days = min($this->effective_days, 4);
        $rates = [1 => 0.25, 2 => 0.50, 3 => 0.75, 4 => 1.00];
        $rate = $rates[$days] ?? 1.00;
        $charged = $this->effective_days * $rate;

        return [
            'rate' => $rate,
            'days_charged' => $charged,
            'computation' => "{$this->effective_days} days × {$rate} = {$charged} days"
        ];
    }
